...
public class CreditCardFormatTextWatcher extends FormTarjetaActivity implements TextWatcher {

    public static final int NO_MAX_LENGTH = -1;
    private int maxLength = NO_MAX_LENGTH;
    private int paddingPx;
    private boolean internalStopFormatFlag;
    private TextView textViewNumeroTarjeta;
    ...
    public CreditCardFormatTextWatcher(@NonNull TextView textView) {
        ...
        this.textViewNumeroTarjeta = textView;
    ...
    public void setPaddingEm(@NonNull TextView textView, float em) {
        double emSize = textView.getPaint().measureText("x");
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        if (s.length() == 16) {
            textViewNumeroTarjeta.setText(s.toString().substring(0, 4) + "  " + s.toString().substring(4, 8) + "  " + s.toString().substring(8, 12) + "  " + s.toString().substring(12, 16));
        }
        if (s.length() < 16) {
            textViewNumeroTarjeta.setText(s);
        }
    }
